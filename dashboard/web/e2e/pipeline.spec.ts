...
test(`submit ${TEMPLATE} and follow it to completion`, async ({ page }) => {
  ...
  while (Date.now() < deadline && !TERMINAL.includes(status)) {
    ...
    const wf = await page.evaluate(async (wid) => (await fetch(`/api/workflows/${wid}`)).json(), id);
    status = wf.status;
    const line = `${status} | ${(wf.tasks ?? []).map((t: { name: string; phase: string }) => `${t.name}=${t.phase}`).join(' ')}`;
    if (line !== lastLine) {
  ...
  const detail = await page.evaluate(async (wid) => (await fetch(`/api/workflows/${wid}`)).json(), id);
  console.log('final', JSON.stringify({ status: detail.status, tasks: (detail.tasks ?? []).map((t: { name: string; phase: string; message?: string; outputPath?: string }) => ({ name: t.name, phase: t.phase, message: t.message, outputPath: t.outputPath })) }, null, 2));
