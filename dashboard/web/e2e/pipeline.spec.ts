 ...
 *   DASHBOARD_PIPELINE_LIVE=1 PIPELINE_ACKNOWLEDGE_PREFLIGHT=1 \
 *   PIPELINE_TEMPLATE=mujoco-pipeline PIPELINE_OVERRIDES='{"total_steps":"512","num_envs":"1","episodes":"2"}' PIPELINE_TIMEOUT_MIN=30 \
 ...
 */
const TEMPLATE = process.env.PIPELINE_TEMPLATE ?? 'mujoco-pipeline';
...
test(`submit ${TEMPLATE} and follow it to completion`, async ({ page }) => {
  ...
  expect(status, `workflow ${id} ended ${status}`).toBe('SUCCEEDED');
    ...
    if (!EXISTING && id && !TERMINAL.includes(status)) {
      const cleanupStatus = await page.evaluate(async (wid) => (await fetch(`/api/workflows/${wid}/cancel`, { method: 'POST' })).status, id);
      expect(cleanupStatus, 'Owned unfinished pipeline must accept cancellation').toBe(202);
    }
  }
});
