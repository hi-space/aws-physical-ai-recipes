...
export const BUILTIN_TEMPLATES: Template[] = [
  ...
  recipe({ id: 'gr00t-finetune', title: "GR00T N1.6 파인튜닝", category: 'training', mlflow: true,
    ...
    tasks: [{ name: 'finetune', resource: 'gpu', image: '{{ image }}', command: ['python', '/opt/recipes/groot/train.py'],
      args: ['--output-dir', '{{output}}', '--seed', '{{ seed }}', '--resume', '{{ resume }}', '--base-model-path', '{{ base_model }}',
        '--dataset-path', '{{input:0}}', '--embodiment-tag', 'NEW_EMBODIMENT', '--modality-config-path', '/opt/recipes/groot/so101_modality.py',
        '--max-steps', '{{ max_steps }}', '--save-steps', '{{ save_steps }}', '--global-batch-size', '{{ batch_size }}', '--num-gpus', '1'],
  ...
  recipe({ id: 'leisaac-evaluate', title: "LeIsaac + GR00T 폐루프 평가", category: 'evaluation',
    ...
    params: [image('GROOT_RUNTIME_IMAGE_URI', 'policy_image'), image('LEISAAC_IMAGE_URI', 'sim_image'), dataset('dataset_name', 'groot-checkpoints-run-id'),
      P('checkpoint_dir', "데이터셋 안의 체크포인트 폴더", 'checkpoint-100'),
      P('episodes', "평가 회차 수", '5', 'number'), seed(), token()],
    groups: [{ name: 'evaluation', barrier: true, ignoreNonleadStatus: false, tasks: [
      { name: 'policy', resource: 'gpu', image: '{{ policy_image }}', command: ['python', '-m', 'gr00t.eval.run_gr00t_server'],
        args: ['--model-path', '{{input:0}}/{{ checkpoint_dir }}', '--embodiment-tag', 'NEW_EMBODIMENT', '--host', '0.0.0.0', '--port', '5555'],
      ...
      { name: 'evaluate', lead: true, resource: 'gpu', image: '{{ sim_image }}', command: ['/isaac-sim/python.sh', '/opt/recipes/leisaac/evaluate.py'],
        args: ['--checkpoint', '{{input:0}}/{{ checkpoint_dir }}', '--policy-host', '{{host:policy}}', '--output-dir', '{{output}}',
          '--seed', '{{ seed }}', '--episodes', '{{ episodes }}', '--headless', '--enable_cameras'], environment: isaacEnv,
