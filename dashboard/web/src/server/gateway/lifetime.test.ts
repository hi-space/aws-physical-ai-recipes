...
it('fails closed when an active connection authorization check never completes', async () => {
  ...
  const { cookie } = await consumeTicket(launch.ticket, launch.host, { repo });
  const controller = new AbortController();
  const cleanup = guardConnection(session, cookie.split(';')[0], launch.host, controller, { repo, recheckMs: 20 });
