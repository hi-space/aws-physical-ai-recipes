...
it('requires project write permission for retry even when the user is a global researcher', async () => {
  ...
  await repo.kv.put({ pk: 'PROJECT#p', sk: 'META', id: 'p', members: { alice: 'viewer' }, gsi1pk: 'TYPE#PROJECT', gsi1sk: 'p' });
  await repo.kv.put({ pk: 'WF#w', sk: 'META', id: 'w', owner: 'bob', projectId: 'p' });
  const principal = { user: 'alice', subject: 'alice', email: '', role: 'researcher' as const };
