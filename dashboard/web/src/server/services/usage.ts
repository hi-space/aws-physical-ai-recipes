...
export interface TaskUsage {
  ...
  timingBasis: 'runtime-receipts' | 'task-observation' | 'not-started' | 'unknown';
}
...
export function estimateRunUsage(workflow: Workflow, tasks: Task[], runtime: Item[], now: Date) {
...
export async function runUsage(id: string, session: Session, repo: Repo = getRepo(), now = () => new Date()) {
  ...
  return estimateRunUsage(workflow, tasks, runtime, now());
}
