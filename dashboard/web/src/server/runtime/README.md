...
| POST `/runtime/uploads` | purpose=checkpoint, destination, files | exact Go `{ uploads: [{ path, url, headers }] }` |
| POST `/runtime/uploads/complete` | same canonical file manifest | verified immutable READY receipt |
| GET `/runtime/inputs` | task capability | pinned dataset input manifests, checksums and version-specific download URLs |
| GET `/runtime/checkpoints?replica=N` | current task capability | committed prior-attempt checkpoint descriptors and fresh version-pinned GET URLs |
