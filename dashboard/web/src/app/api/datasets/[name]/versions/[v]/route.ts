import { versionNumber } from '@/server/data/versions';
import { z } from 'zod';
...
export const GET = route<{ name: string; v: string }>('viewer', async ({ params, url }) => listFiles(params.name, versionNumber(params.v), q(url, 'prefix') ?? '', q(url, 'token')));
export const POST = route<{ name: string; v: string }>('researcher', async ({ params, req, session }) => {
  ...
  if (b.action === 'tags') return setVersionTags(params.name, versionNumber(params.v), b.tags ?? []);
  return refreshSize(params.name, versionNumber(params.v));
