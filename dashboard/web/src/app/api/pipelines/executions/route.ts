...
export const POST = route('researcher', async ({ req, session }) => {
  const b = await body(req, z.object({
    parameters: z.record(z.string(), z.string()), displayName: z.string().optional(),
    expectedPipelineArn: z.string().min(1).max(2048).optional(),
    expectedOwnerSubject: z.string().min(1).max(256).optional(),
  }));
