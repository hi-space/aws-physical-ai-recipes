...
import { controllerStatus } from '@/server/workflow/controller';
...
export const GET = route('admin', async () => {
  ...
  return { config: config(), env, settings: await getRepo().getSettings(), controller: controllerStatus(), lease: await getRepo().getLease('CONTROLLER') };
});
