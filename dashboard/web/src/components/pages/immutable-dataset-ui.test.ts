...
it('exposes downloads only for verified immutable listings, not pending mutable files',()=>{
 const render=(state:'PENDING'|'READY',immutable:boolean)=>{
  ...
  client.setQueryData(['api','/api/datasets/data/versions/1?prefix='],{immutable,entries:[{name:'file',key:'projects/p/v1/file',path:'file',versionId:'old',size:3,isPrefix:false}]});
