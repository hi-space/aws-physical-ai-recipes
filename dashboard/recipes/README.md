...
| `cosmos3/Dockerfile` | `COSMOS3_IMAGE_URI` | Pinned NVIDIA cosmos-framework (release 2026-09-18, commit `c23e51f`) on the NGC CUDA 12.8 cudnn-devel base, uv-managed Python 3.13, `uv sync --locked --all-extras --group=cu128`; serves both Cosmos3-Edge (4B) and Cosmos3-Nano (16B) via `--checkpoint-path`. **Nano depth transfer verified 2026-09-20 on HyperPod ml.g6e.4xlarge (L40S 48 GB): 93 frames @480p, UniPC 35 steps ≈ 2.5 min sampling, ≈ 33 GB VRAM at load, whole task ≈ 10 min including the 29 GB checkpoint download.** Weights are cached by the adapter under the project's FSx root (`/fsx/checkpoints/projects/<project>/cache/hf`, outside the published output; `HF_HOME` overrides) because a container-layer download on the node's 100 GB root disk gets the pod evicted. cosmos-framework's content guardrails (`guardrails=on`, default) download the gated `nvidia/Cosmos-Guardrail1` checkpoint, so the HF token's account must have accepted that license once; `guardrails=off` passes `--no-guardrails`. Neither recipe pins an instance type by default (`cosmos_platform` empty = any GPU node the queue offers) — Kueue counts GPUs, not VRAM, so pin Nano to a 48 GB type (e.g. `ml.g6e.4xlarge`) on mixed clusters. 720p needs an 80 GB-class GPU (upstream ≈ 46 GiB peak). Edge on a 24 GB GPU (A10G/L4) is untested. |
...
| `replicator-sdg` | Real USD scene, seeded camera randomization, RGB / metric-depth / semantic frames, strict frame manifest. Kit runs with `--portable-root` under `XDG_CACHE_HOME` (as uid 1000 the root-owned `/isaac-sim/kit/cache` is unwritable and the RTX renderer never starts) and a first-frame watchdog (`--first-frame-timeout`, default 900 s) fails the task instead of spinning forever. Lights/camera are authored on the stage, not inside `rep.new_layer()` (prims in that sublayer never reached the headless RTX renderer: dome light present, RGB pass black, depth/segmentation fine); the recipe warms up (`--warmup-updates` 200) and fails on a black RGB pass (`--min-rgb-mean` 2), and exceptions exit non-zero before `SimulationApp.close()` (which otherwise ends the process with status 0). Verified 2026-09-20 on ml.g6e.4xlarge: 93 frames in ~4 min including Kit startup; RGB mean ≈ 240 after the layer fix. |
...
| `cosmos3-nano-pipeline` | SDG → depth control video → Cosmos3-Nano `video2video` transfer (`depth.control_path`, `control_guidance`). One output frame per SDG frame; same fixed 0–5 m depth visualization and non-trajectory disclaimer as `cosmos-pipeline`. Verified end to end 2026-09-20 (run `37e29fc0b05f6643`, ml.g6e.4xlarge, `guardrails=off`): 736×544 h264, 16 fps, 93 frames, 9.8 MB. |
