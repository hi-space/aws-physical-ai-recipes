        ...
        while True:
            ...
            if not (response.headers.get('Content-Type') or '').startswith('text/event-stream'):
                ...
                if page.get('source') == 'none':
                    ...
                    print('No Pod for this task is available in Kubernetes; logs are not stored by the dashboard.', file=sys.stderr); return
                ...
                for raw in response:
                    ...
                    if line == '':
                        if event == 'line':
                            ...
                            raise CliError('Log stream stopped: authorization is no longer valid.')
