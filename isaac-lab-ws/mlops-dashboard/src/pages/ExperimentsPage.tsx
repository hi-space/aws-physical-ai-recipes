...
import { mockTrainingMetrics } from '../data/mockWorkers';

export default function ExperimentsPage() {
  const { experiments, workers } = useOutletContext<OutletContextType>();
  const [selectedIds, setSelectedIds] = useState<string[]>(experiments.map((e) => e.id));
  ...
  // Collect all hyperparameter keys across selected experiments
  ...
  const allMetrics = mockTrainingMetrics.filter((m) => {
        ...
        {experiments.map((exp, idx) => {
          ...
          return (
            <div
              ...
              className={`bg-white rounded-lg shadow-sm p-5 cursor-pointer transition-all border-2 ${
                isSelected ? 'border-blue-500 ring-1 ring-blue-200' : 'border-transparent hover:border-gray-200'
              }`}
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span
                    ...
                    style={{ backgroundColor: expColors[idx % expColors.length] }}
                  />
              ...
              <dl className="grid grid-cols-2 gap-y-1.5 text-sm">
                <div>
                  <dt className="text-xs text-gray-400">Algorithm</dt>
                  <dd className="font-medium text-gray-700">{exp.algorithm}</dd>
                </div>
                <div>
                  <dt className="text-xs text-gray-400">Workers</dt>
                  <dd className="font-medium text-gray-700">{expWorkers.length}</dd>
                </div>
                <div>
                  <dt className="text-xs text-gray-400">Task</dt>
                  <dd className="text-gray-600 text-xs truncate">{exp.taskName}</dd>
                </div>
                <div>
                  <dt className="text-xs text-gray-400">Best Reward</dt>
                  <dd className="font-medium text-gray-700">{exp.bestReward.toFixed(1)}</dd>
                </div>
              </dl>
              ...
              <div className="mt-3">
                <div className="flex justify-between text-xs text-gray-400 mb-1">
                  <span>Progress</span>
                  <span>{exp.currentStep}/{exp.totalSteps}</span>
                </div>
      ...
      {allMetrics.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm p-5">
          ...
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full" preserveAspectRatio="xMidYMid meet">
            ...
            {/* Lines per experiment */}
            {selectedExps.map((exp) => {
              const metrics = allMetrics.find((m) => exp.workerIds.includes(m.workerId));
              if (!metrics) return null;
              const toX = (i: number) => padL + (i / (metrics.rewards.length - 1)) * chartW;
              const toY = (v: number) => padT + chartH - ((v - minReward) / rewardRange) * chartH;
              const d = metrics.rewards
              ...
              return (
                <path
                  ...
                  d={d}
                  fill="none"
                  stroke={expColors[experiments.indexOf(exp) % expColors.length]}
                  strokeWidth={2}
                  strokeLinecap="round"
            ...
            {selectedExps.map((exp) => (
              <span key={exp.id} className="flex items-center gap-1.5 text-xs text-gray-500">
                <span
                  ...
                  style={{ backgroundColor: expColors[experiments.indexOf(exp) % expColors.length] }}
                />
                {exp.name}
              </span>
            ))}
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  {selectedExps.map((exp) => (
                    <th key={exp.id} className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">
                      {exp.name}
                    </th>
                  ))}
                </tr>
                  ...
                  return (
                    <tr key={key} className={allSame ? '' : 'bg-yellow-50'}>
                      ...
                      {values.map((v, i) => (
                        <td
                          key={i}
                          className={`px-4 py-2 text-sm font-mono ${allSame ? 'text-gray-500' : 'text-gray-900 font-semibold'}`}
                        >
