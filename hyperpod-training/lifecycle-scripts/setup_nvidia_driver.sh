...
# 버전(proprietary 커널 모듈)으로 교체한다.
#
# 왜 필요한가:
#   HyperPod AMI의 NVIDIA 드라이버(595.x, Open Kernel Module)에서는 Isaac Sim 4.5/5.1의
#   RTX 렌더러가 초기화(rtx.scenedb.plugin) 중 segfault 로 죽는다. headless 물리 시뮬은
#   정상이지만 뷰포트(DCV GUI)와 카메라 렌더링(--enable_cameras)이 모두 불가능하다.
#   모듈 1 DCV 인스턴스와 동일한 580.173.02(proprietary)에서는 정상 동작한다.
#
# 동작:
#   - GPU 가 없는 노드(head)는 건너뛴다.
#   - 이미 목표 버전이면 건너뛴다.
#   - GPU 를 잡고 있는 서비스(DCGM, 헬스 모니터링 에이전트, persistenced, DCV)를 잠시 멈추고
#     커널 모듈을 내린 뒤 NVIDIA .run 설치기로 같은 사용자공간+proprietary 커널 모듈을
#     빌드/설치하고(약 3~5분), 모듈을 다시 올리고 서비스를 재시작한다.
...
MODULE_TYPE="$(grep -o 'Open Kernel Module' /proc/driver/nvidia/version 2>/dev/null || echo proprietary)"
if [ "${CURRENT}" = "${NVIDIA_DRIVER_VERSION}" ] && [ "${MODULE_TYPE}" = "proprietary" ]; then
  log "Driver ${CURRENT} (proprietary) already installed."; exit 0
fi
log "Current driver: ${CURRENT} (${MODULE_TYPE}) → target ${NVIDIA_DRIVER_VERSION} (proprietary)"
...
[ -d extracted/kernel ] || { log "WARNING: runfile has no proprietary kernel module; keeping current driver."; exit 0; }
...
restore_services() {
  ...
  modprobe nvidia_drm 2>/dev/null; modprobe gdrdrv 2>/dev/null; modprobe nvidia_peermem 2>/dev/null; modprobe efa_nv_peermem 2>/dev/null
...
if lsmod | grep -qE "^nvidia "; then
  log "WARNING: nvidia module still in use ($(fuser -v /dev/nvidia* 2>&1 | awk 'NR>1{print $NF}' | sort -u | tr '\n' ' ')); keeping current driver."
  restore_services; exit 0
fi

# --- 설치 ----------------------------------------------------------------------
log "Installing NVIDIA ${NVIDIA_DRIVER_VERSION} (proprietary kernel module)..."
( cd extracted && ./nvidia-installer -s --ui=none --no-questions --kernel-module-type=proprietary \
