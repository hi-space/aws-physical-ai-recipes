...
export interface ContainerBuildProjectsProps {
  alias: string;
  role: iam.IRole;
  trainingRepository: ecr.IRepository;
  inferenceRepository: ecr.IRepository;
  bucketName: string;
...
/**
 * 학습/추론 이미지 빌드용 CodeBuild 프로젝트 2개.
 *   - PrivilegedMode (Docker in Docker), aws/codebuild/standard:7.0.
 *   - repositoryUrl 비어 있으면 NO_SOURCE 모드 (S3 zip 업로드).
 */
export class ContainerBuildProjects extends Construct {
  public readonly trainingProject: codebuild.Project;
  public readonly inferenceProject: codebuild.Project;

  constructor(scope: Construct, id: string, props: ContainerBuildProjectsProps) {
    super(scope, id);

    const stack = cdk.Stack.of(this);
    const suffix = props.alias ? `-${props.alias}` : '';
    ...
    this.trainingProject = new codebuild.Project(this, 'TrainingBuild', {
      projectName: `groot-n16-training-build${suffix}`,
      ...
      logging: {
        cloudWatch: {
          ...
          logGroup: new logs.LogGroup(this, 'TrainingLogGroup', {
            logGroupName: `/aws/codebuild/groot-n16-training-build${suffix}`,
    ...
    // ---------------- Inference ----------------
    this.inferenceProject = new codebuild.Project(this, 'InferenceBuild', {
      projectName: `groot-n16-inference-build${suffix}`,
      description: 'GR00T-N1.6 추론 컨테이너 빌드 및 ECR 푸시',
      role: props.role,
      ...(baseSource ? { source: baseSource } : {}),
      buildSpec: props.repositoryUrl
        ? codebuild.BuildSpec.fromSourceFilename('container/inference/buildspec.yml')
        : noSourceBuildSpec,
      environment: {
        buildImage: codebuild.LinuxBuildImage.STANDARD_7_0,
        computeType: codebuild.ComputeType.LARGE,
      ...
      environmentVariables: {
        ECR_REGISTRY: { value: ecrRegistry },
        AWS_DEFAULT_REGION: { value: stack.region },
        IMAGE_REPO: { value: props.inferenceRepository.repositoryName },
      },
      logging: {
        cloudWatch: {
          enabled: true,
          logGroup: new logs.LogGroup(this, 'InferenceLogGroup', {
            logGroupName: `/aws/codebuild/groot-n16-inference-build${suffix}`,
            retention: logs.RetentionDays.ONE_MONTH,
            removalPolicy: cdk.RemovalPolicy.DESTROY,
          }),
        },
