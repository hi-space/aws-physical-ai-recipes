...
```bash
docker build -t groot-n1:latest /home/ubuntu/environment/groot_docker
...
echo "=== GR00T Container ==="
docker ps --filter name=groot-inference --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'
...
빌드 명령 출력을 직접 확인한다 (`docker build` 콘솔 로그).
...
```bash
docker logs groot-inference 2>&1 | tail -200
```
...
# 2. 빌드 재시작
docker build --no-cache -t groot-n1:latest /home/ubuntu/environment/groot_docker
```
...
| Symptom | Check | Fix |
|---------|-------|-----|
| Connection refused | `ss -tlnp \| grep 5555` | 추론 컨테이너 미기동: `docker ps \| grep groot-inference` 확인 후 `docker run ...` 재실행 |
