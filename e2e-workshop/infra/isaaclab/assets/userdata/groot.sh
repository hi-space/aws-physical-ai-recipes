...
# =============================================================================
# NOTE: 이 스크립트는 더 이상 UserData에서 자동 실행되지 않습니다.
#       워크숍에서는 사용자가 수동으로 docker build/run을 수행합니다.
#       자동 docker build + systemd 등록이 필요한 경우 dcv-instance.ts의
#       UserData에 `source /tmp/userdata-scripts/groot.sh` 라인을 추가해 사용하세요.
#       모델 가중치(EFS) 다운로드는 efs-mount.sh로 이동되었습니다.
#
