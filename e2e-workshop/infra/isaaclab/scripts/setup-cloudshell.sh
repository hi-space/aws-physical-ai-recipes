...
echo ""
echo " 배포 명령어:"
echo "   npx cdk deploy -c vpcCidr=10.0.0.0/16 -c versionProfile=latest -c region=us-east-1 --require-approval never"
echo ""
echo " 세션 끊김 방지 (권장):"
echo "   nohup npx cdk deploy -c vpcCidr=10.0.0.0/16 -c versionProfile=latest -c region=us-east-1 --require-approval never > deploy.log 2>&1 &"
