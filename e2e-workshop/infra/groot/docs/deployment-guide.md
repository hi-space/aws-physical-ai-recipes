...
1. **infra/isaaclab 스택이 배포된 상태**여야 합니다
...
- **`GrootBatchTrainShared`** — ECR + CodeBuild. 계정당 1번만 배포 (워크숍 시작 전 관리자가 수행).
...
`-c userId=<userId>` 만 지정하면 부모 IsaacLab 스택을 자동으로 조회해 VPC/EFS/Subnet 정보를 가져옵니다:

```bash
CDK_DEFAULT_REGION=ap-northeast-2 npx cdk deploy GrootBatchTrain -c userId=yoo
```
...
사용자 본인이 만든 Batch Compute Env / Queue / Job Definition 만 삭제합니다 (공유 ECR/CodeBuild 는 다른 사용자가 쓰므로 그대로 둡니다):

```bash
CDK_DEFAULT_REGION=ap-northeast-2 npx cdk destroy GrootBatchTrain -c userId=yoo
```

> **공유 스택**: `GrootBatchTrainShared` (ECR + CodeBuild) 는 워크숍 종료 후 관리자가 정리합니다. `cdk destroy GrootBatchTrainShared` 한 번이면 ECR 이미지까지 함께 삭제됩니다 (Custom Resource 의 onDelete 핸들러가 `ecr deleteRepository --force` 호출).
