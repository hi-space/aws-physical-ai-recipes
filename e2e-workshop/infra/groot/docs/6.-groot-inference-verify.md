...
- DCV 인스턴스 접속 가능
- Docker 이미지 `groot-batch-train:latest`가 ECR에 존재
...
# 이미지 Pull
docker pull <AccountId>.dkr.ecr.ap-northeast-2.amazonaws.com/groot-batch-train:latest
```
...
docker run -d --gpus all \
  ...
  -p 5555:5555 \
  <AccountId>.dkr.ecr.ap-northeast-2.amazonaws.com/groot-batch-train:latest \
...
docker pull $ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/groot-batch-train:latest
```
...
docker run -d --gpus all \
  ...
  -e HF_TOKEN=<your-huggingface-token> \
  -p 5555:5555 \
  $ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/groot-batch-train:latest \
  python -m gr00t.eval.run_gr00t_server \
    --model-path /mnt/efs/gr00t/checkpoints \
    --embodiment-tag new_embodiment
...
# 테스트 스크립트 실행
python3 ~/aws-physical-ai-recipes/e2e-workshop/groot/inference/batch-zmq/test_inference.py
```
