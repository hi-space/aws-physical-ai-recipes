...
- DCV 인스턴스 접속 가능
- Docker 이미지 `groot-runtime:latest`가 ECR에 존재
...
# 이미지 Pull
docker pull <AccountId>.dkr.ecr.ap-northeast-2.amazonaws.com/groot-runtime:latest
```
...
docker run -d --gpus all \
  ...
  -p 5555:5555 \
  <AccountId>.dkr.ecr.ap-northeast-2.amazonaws.com/groot-runtime:latest \
...
docker pull $ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/groot-runtime:latest
```
...
docker run -d --gpus all \
  ...
  -e HF_TOKEN=<your-huggingface-token> \
  -p 5555:5555 \
  $ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/groot-runtime:latest \
  python -m gr00t.eval.run_gr00t_server \
    --model-path /mnt/efs/gr00t/checkpoints \
    --embodiment-tag new_embodiment
