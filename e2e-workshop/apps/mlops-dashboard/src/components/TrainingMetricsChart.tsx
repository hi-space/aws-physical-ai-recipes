...
export default function TrainingMetricsChart({ metrics }: { metrics: TrainingMetrics | undefined }) {
  ...
  if (!metrics) {
    return (
      <div className="bg-white rounded-lg shadow-sm p-5">
        ...
        <div className="flex items-center justify-center h-[200px] text-gray-400 text-sm">No training metrics available</div>
      </div>
    );
  }

  const dataMap: Record<MetricTab, number[]> = {
    reward: metrics.rewards, policyLoss: metrics.policyLoss, valueLoss: metrics.valueLoss, episodeLength: metrics.episodeLengths,
  };
  const data = dataMap[activeTab];
  const steps = metrics.steps;
  const currentTab = tabs.find((t) => t.key === activeTab)!;

  const W = 560, H = 180, padL = 50, padR = 15, padT = 10, padB = 25;
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-5">
        ...
        <div className="flex gap-1">
          {tabs.map((tab) => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`px-2.5 py-1 rounded text-xs font-medium transition-colors ${activeTab === tab.key ? 'text-white' : 'text-gray-500 bg-gray-100 hover:bg-gray-200'}`}
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" preserveAspectRatio="xMidYMid meet">
        {yLabels.map((v, i) => {
          const y = toY(v);
          return (<g key={i}><line x1={padL} y1={y} x2={W - padR} y2={y} stroke="#e5e7eb" strokeWidth={1} />
            <text x={padL - 6} y={y + 3} textAnchor="end" className="fill-gray-400" style={{ fontSize: '10px' }}>{v >= 100 ? v.toFixed(0) : v.toFixed(2)}</text></g>);
        })}
        {[0, Math.floor(steps.length / 2), steps.length - 1].map((idx) => (
          <text key={idx} x={toX(idx)} y={H - 4} textAnchor="middle" className="fill-gray-400" style={{ fontSize: '10px' }}>{steps[idx]}</text>
        ))}
        <path d={pathD} fill="none" stroke={currentTab.color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
        <path d={`${pathD} L${toX(data.length - 1).toFixed(1)},${(padT + chartH).toFixed(1)} L${padL},${(padT + chartH).toFixed(1)} Z`} fill={currentTab.color} opacity={0.08} />
      </svg>
