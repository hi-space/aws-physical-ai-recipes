...
- ECR 리포지토리 2개 (`groot-sm-training`, `groot-sm-inference`)
- CodeBuild 프로젝트 2개
...
- ECR: `groot-sm-training-alice`, `groot-sm-inference-alice`
- IAM: `GR00TSageMakerRole-alice`, `GR00TCodeBuildRole-alice`
- CodeBuild: `groot-sm-training-build-alice`, `groot-sm-inference-build-alice`
- SageMaker Endpoint: `groot-sm-endpoint-alice`
- Model Package Group: `groot-sm-models-alice`
- SageMaker Pipeline: `groot-sm-finetuning-alice`
...
발급된 URL을 브라우저에 붙여 넣으면 experiment `groot-sm-finetune` 아래 run 별로 metric/param 233개+ / artifact (checkpoint safetensors + DeepSpeed optimizer states) 확인 가능. S3 backing store는 `s3://<bucket>/mlflow-artifacts/`.
...
콘솔: SageMaker → Model Registry → groot-sm-models[-{alias}] → 최신 버전 → Update Status → Approved
...
| SageMaker Training Job — S3 채널 | ✅ Completed (job `groot-sm-training-2026-05-12-04-16-10-593`, ml.g5.12xlarge × 1 instance, 4 GPU, batch_size 32, 100 step, 1655s). model.tar.gz 123MB → S3. |
...
| CodeBuild 실패 | flash-attn 다운로드 실패 등 | CloudWatch `/aws/codebuild/groot-sm-training-build` 확인 후 재시도 |
