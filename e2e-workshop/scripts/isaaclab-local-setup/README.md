...
| 작업 디렉토리 | `~/environment/IsaacLab` (`develop` 브랜치) |
...
IsaacLab `develop` 브랜치도 전체적으로 3.11을 전제합니다:
...
`develop` 브랜치의 AppLauncher는 기본적으로 Standalone 모드(SimulationApp 없이)로 진입합니다. 그러나 `isaaclab.envs` 등이 `pxr` (USD) 모듈을 무조건 import하므로, SimulationApp이 초기화되지 않으면 `No module named 'pxr'` 에러가 발생합니다. `LAUNCH_OV_APP=1`로 SimulationApp 모드를 강제 활성화하면 해결됩니다.
