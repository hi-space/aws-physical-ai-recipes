...
# 리전 지정
./scripts/check-quotas.sh -n 20 -r us-west-2 --auto-request
```
...
# 9. code-server (VSCode) 비활성화 (CloudFront + code-server 미설치)
...
# 11. Isaac Sim 버전 오버라이드 (stable 프로필 + Isaac Sim 5.1.0)
...
- `~/aws-physical-ai-recipes/isaac-lab-workshop/infra-multiuser-groot`로 cd
