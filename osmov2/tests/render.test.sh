...
assert_not_contains "${spot_out}" '"        - key: node.kubernetes.io/instance-type' 'zone value must not glue onto instance-type key'
printf -v _expected '              - "ap-northeast-2a"\n        - key: node.kubernetes.io/instance-type'
assert_contains "${spot_out}" "${_expected}" 'instance-type key on its own line after zone'
