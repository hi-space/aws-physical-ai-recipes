...
- [workflows/README.md](workflows/README.md): upstream cookbook README copied
  from NVIDIA OSMO.
- [validation.md](validation.md): validation result, runtime notes, plots, and
  retained artifact references.
- [validation/](validation/): small visual and metrics artifacts retained in git.
...
- target the AWS reference architecture's `g7e-rtx-pro-6000` OSMO platform and
...
- collect GR00T training GPU metrics, TensorBoard loss, a run manifest, and
...
Use `NUT_POURING_PREPARE_ONLY=true` and `NUT_POURING_PREPARED_WORKFLOWS_DIR=...`
to refresh the prepared workflow set without submitting jobs.
