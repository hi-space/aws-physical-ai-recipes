...
The status of this fix in OSMO 6.3.1 is unconfirmed and will be verified during the live-cluster re-validation. Until confirmed resolved, this repo continues to avoid the problematic upstream path by installing the Helm charts directly with AWS-owned values and explicit bootstrap steps:
...
OSMO 6.3 emits `scheduling.run.ai/v2alpha2` `PodGroup` objects from the KAI scheduler path even for the CPU-only smoke workflow. This repo installs the pinned KAI Scheduler chart by default before OSMO and sets the backend scheduler name to `kai-scheduler`.

KAI Scheduler is pinned to `v0.13.0` for OSMO 6.3 compatibility. `v0.13.0` is also the minimum version named in NVIDIA's April 2026 KAI security bulletin. Upgrade KAI only through an explicit PR that reruns the OSMO smoke workflow through real KAI scheduling.
...
OSMO 6.3 validates workflow resources against capacity already visible to the OSMO backend. Karpenter, by design, creates GPU nodes only after Kubernetes sees pending pods. That means an OSMO workflow that requires a large GPU node can be rejected before Karpenter has a chance to provision the node.
...
## Router Consolidation (OSMO 6.3)

OSMO 6.3 consolidates the router and UI into the unified `service` Helm chart. However, the OSMO 6.3 built-in router is a session router (subdomain-based), not a path-based HTTP router. This repo intentionally keeps the nginx internal router (`osmo-internal-router`) as the single entry point because it provides path-based routing (`/` → API, `/api/logger/` → logger websocket) that the session router cannot replace. The Envoy and oauth2-proxy gateway services remain disabled (private baseline). See `docs/superpowers/plans/2026-07-11-osmo-63-render-findings.md` for detailed router mode analysis.
