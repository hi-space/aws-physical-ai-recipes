...
Runtime validation passed on 2026-05-05 against `aws-osmo-dev-repro-eks` in `ap-northeast-2`, with GPU observability revalidated on 2026-05-06:
...
- Created AMP workspace `ws-41a61aa8-e5cb-4196-aa2f-12abae537904` with alias `aws-osmo-dev-repro-eks-observability`.
- Configured Prometheus `remoteWrite` with SigV4 to `https://aps-workspaces.ap-northeast-2.amazonaws.com/workspaces/ws-41a61aa8-e5cb-4196-aa2f-12abae537904/api/v1/remote_write`.
...
- Created AMG workspace `g-9d381a8099`, endpoint `https://g-9d381a8099.grafana-workspace.ap-northeast-2.amazonaws.com`, with AMP data source `AMP aws-osmo-dev-repro-eks`.
...
- Revalidated the dashboard label contract after switching DCGM to Prometheus Operator default label handling: AMP returned `count(kube_pod_info{cluster="aws-osmo-dev-repro-eks"})=48`, `count(up{cluster="aws-osmo-dev-repro-eks",namespace="osmo"})=5`, and `DCGM_FI_DEV_GPU_UTIL{cluster="aws-osmo-dev-repro-eks",exported_namespace="osmo-workflows"}` for a GPU pod in `osmo-workflows`.
...
Committed GPU panel captures and the validation summary are under [infra/observability/validation](../infra/observability/validation/). To reproduce those views, open the AMG `AWS OSMO Overview` dashboard, set the time range around the GPU workflow run, and use the GPU panels that query `exported_namespace="osmo-workflows"` DCGM metrics.
