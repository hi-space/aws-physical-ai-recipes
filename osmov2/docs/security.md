...
- OSMO image pulls use an NGC Kubernetes image pull secret created by `scripts/deploy-osmo.sh`; the key is read from `NGC_API_KEY` or `NGC_API_KEY_FILE` and is not stored in Terraform state.
- OSMO 6.3 workflow runtime storage still requires an access-key credential; Terraform creates a least-privilege IAM user scoped to the artifact bucket and KMS key, stores the secret in AWS Secrets Manager, and the deploy wrapper writes it into OSMO's encrypted workflow config.
