...
variable "name_prefix" {
  description = "Name prefix for observability resources. Defaults to the core cluster name when deployed through scripts/deploy-observability.sh."
  type        = string
  default     = "aws-osmo-dev-repro-eks"
}
...
variable "grafana_datasource_name" {
  description = "Prometheus data source name created in AMG by scripts/deploy-observability.sh."
  type        = string
  default     = "AMP aws-osmo"
}

variable "grafana_datasource_uid" {
  description = "Prometheus data source UID created in AMG by scripts/deploy-observability.sh."
  type        = string
  default     = "aws-osmo-amp"
}

variable "grafana_provisioner_service_account_name" {
  description = "AMG service account used by scripts/deploy-observability.sh for data source and dashboard provisioning."
  type        = string
